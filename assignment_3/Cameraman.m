...
folder = fileparts(which('cameraman.png')); % Determine where demo folder is (works with all versions).
baseFileName = 'cameraman.png';
...
grayImage = grayImage - amplitudeImage3;
figure(2); imshow(grayImage, [0 255]);

figure(21); imshow(oldSpikes); axis on;
title('Bright spikes with central spike', 'FontSize', fontSize);

figure(23); imshow(brightSpikes); axis on;
title('Bright spikes other than central spike', 'FontSize', fontSize);
% set(gcf, 'units','normalized','outerposition',[0 0 1 1]); % Maximize figure.
% % 